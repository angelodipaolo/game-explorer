...
| **Facts** | a copy | `GET /api/games/:id/facts` · `GET /api/enrichment/gaps` | `POST /api/enrichment/runs/:runId/facts` — cited, inside a run. (`PUT /api/games/:id/facts` writes `source: "manual"`; that is the owner's hand-entry path, not yours) | `gx facts gaps\|list\|write` |
| **Tags** | a copy | `GET /api/games/:id/tags` · `GET /api/tags` (every spelling in use) | `POST /api/enrichment/runs/:runId/tags` — cited, inside a run · `PUT`/`DELETE /api/games/:id/tags` for one by hand | `gx tags list\|on\|write\|add\|remove` |
